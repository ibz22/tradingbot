"""
Abstract and concrete data gateways for financial statement retrieval.

The ``DataGateway`` interface defines the methods required by the halal
screening engine to fetch financial information about a company.  Concrete
implementations wrap third party APIs.  ``FMPGateway`` provides a default
integration with the Financial Modeling Prep API including rudimentary
caching and rate limiting.  Responses are cached in a lightweight SQLite
database so that repeated screenings can be run offline.
"""
...
class FMPGateway(DataGateway):
    ...
    def __init__(
        self,
        api_key: str,
        cache_db: str = "fmp_cache.db",
        cache_expiry: int = 60 * 60 * 24,
    ...
    async def get_statement(self, ticker: str) -> Dict[str, Any]:
        ...
        income_data, balance_data = await asyncio.gather(
            self._rate_limited_get(income_url), self._rate_limited_get(balance_url)
        )
